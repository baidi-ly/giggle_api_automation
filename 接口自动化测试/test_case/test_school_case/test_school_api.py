...
class TestSchoolApi:
    ...
    def test_school_required_groups_groupCount(self, desc, value, code):
        """创建学生分组-必填字段测试(req)"""
        if desc == 'missing':
            pl = {'pop_items': 'groupCount'}
        else:
            pl = {'groupCount': value}
    ...
    def test_school_format_groups_groupCount(self, desc, value, code, code_res):
        """创建学生分组-数据格式测试(req)"""
        try:
            res = self.school.groups(self.authorization, req=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def test_school_required_putGroups1_req(self, desc, value, code):
        """更新课堂学生默认分组-必填字段测试(req)"""
        if desc == 'missing':
            pl = {'pop_items': 'req'}
        else:
            pl = {'req': value}
        res = self.school.update_groups(authorization=self.authorization, **pl, code=code)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        if code == 500:
            assert res['code'] == 500, f"接口返回状态码异常: 预期【500】，实际【{res['code']}】"
            assert res['message'] == 'internal server error', f"接口返回message信息异常: 预期【'internal server error'】，实际【{res['message']}】"
    ...
    def test_school_format_putGroups1_req(self, desc, value, code, code_res):
        """更新课堂学生默认分组-数据格式测试(req)"""
        try:
            res = self.school.update_groups(self.authorization, req=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def create_lesson(self):
        ...
        res_quiz = self.admin.quiz_list(self.authorization)['data']['content']
    ...
    def test_school_format_report_lessonId(self, desc, value, code, code_res):
        """测验结果上报-数据格式测试(lessonId)"""
        try:
            res = self.school.report(self.authorization, lessonId=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def test_school_boundary_report_lessonId(self, desc, value, code):
        """测验结果上报-边界值测试(lessonId)"""
        res = self.school.report(self.authorization, lessonId=value)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
    ...
    @pytest.mark.release
    def test_school_scenario_report_invalid_lessonId(self):
        """测验结果上报-场景异常-无效的lessonId"""
        lessonId = 999999999
        res = self.school.report(self.authorization, lessonId=lessonId)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    def test_school_format_report_quizId(self, desc, value, code, code_res):
        """测验结果上报-数据格式测试(quizId)"""
        try:
            res = self.school.report(self.authorization, quizId=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def test_school_boundary_report_quizId(self, desc, value, code):
        """测验结果上报-边界值测试(quizId)"""
        res = self.school.report(self.authorization, quizId=value)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == code, f"接口返回状态码异常: 预期【{code}】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    @pytest.mark.release
    def test_school_scenario_report_invalid_quizId(self):
        """测验结果上报-场景异常-无效的quizId"""
        quizId = 999999999
        res = self.school.report(self.authorization, quizId=quizId)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    def test_school_required_report_req(self, desc, value, code):
        """测验结果上报-必填字段测试(req)"""
        if desc == 'missing':
            pl = {'pop_items': 'req'}
        else:
            pl = {'req': value}
        res = self.school.report(authorization=self.authorization, **pl, code=code)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        if code == 500:
            assert res['code'] == 500, f"接口返回状态码异常: 预期【500】，实际【{res['code']}】"
            assert res['message'] == 'internal server error', f"接口返回message信息异常: 预期【'internal server error'】，实际【{res['message']}】"
    ...
    def test_school_format_report_req(self, desc, value, code, code_res):
        """测验结果上报-数据格式测试(req)"""
        try:
            res = self.school.report(self.authorization, req=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def test_school_boundary_report_req(self, desc, value, code):
        """测验结果上报-边界值测试(req)"""
        res = self.school.report(self.authorization, req=value)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == code, f"接口返回状态码异常: 预期【{code}】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    @pytest.mark.release
    def test_school_scenario_report_invalid_req(self):
        """测验结果上报-场景异常-无效的req"""
        req = 'INVALID_VALUE'
        res = self.school.report(self.authorization, req=req)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    def test_school_security_report_req(self, desc, value, code, code_res):
        """测验结果上报-安全测试(req)"""
        res = self.school.report(self.authorization, req=value)
        if code and not code_res:
            assert not res
