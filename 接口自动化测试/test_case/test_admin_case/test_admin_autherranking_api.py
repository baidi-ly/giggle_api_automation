...
class TestAdminAutherRanking:
    ...
    def test_admin_autherranking_positive_calculate_ok(self):
        ...
        assert res['data'] == "任务已触发", f"接口返回data数据异常：{res['data']}"
    ...
    def test_admin_autherranking_permission_calculate(self, desc, value):
        """触发作者排名计算-权限测试"""
        res = self.admin_auther.calculate(value, code=401)
        if res:
            assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
