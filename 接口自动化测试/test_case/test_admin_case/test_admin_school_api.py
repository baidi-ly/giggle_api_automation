...
class TestSchoolApi:
    ...
    def setup_class(self):
        ...
        self.authorization, self.userId = self.admin_school.get_admin_authorization()
    ...
    def school_fixture(self):
        ...
        students_res = self.admin_school.batch_add_student(self.authorization, class_a_id, **pl1)['data']
        ...
        # 批量添加学生
        pl1 = {"studentNames": student_names}
        students_res = self.admin_school.batch_add_student(self.authorization, class_b_id, **pl1)['data']
        ...
        # 批量添加学生
        pl1 = {"studentNames": student_names}
        students_res = self.admin_school.batch_add_student(self.authorization, class_c_id, **pl1)['data']
    ...
    def test_admin_school_positive_admin_create_class(self):
        ...
        for _class in add_list:
            if _class['className'] == className:
                ...
                assert  _class['teacherUserId'] == self.userId
                break
        else:
    ...
    def test_admin_school_class_student_check(self, create_class):
        ...
        students_res = self.admin_school.batch_add_student(self.authorization, class_id, **pl)['data']
        studentIds = DataFrame(students_res).loc[:, "id"].tolist()
        # 创建班级后，获取班级列表，验证添加成功
        add_list = self.admin_school.class_students(self.authorization, class_id)
        for student in add_list:
            if student['className'] == studentIds:
                assert  studentIds['description'] == "三年级历史学习班"
                assert  studentIds['grade'] == 3
                assert  studentIds['subject'] == "History"
                break
        else:
            assert False, "新增班级后，未在返回的班级列表中找到新增得班级！"
        ...
        update_res = self.admin_school.update_student(self.authorization, class_id, **pl)
        assert update_res['message'] == "success"
        # 更新班级信息后，获取班级列表，验证更新成功
        update_list = self.admin_school.class_students(self.authorization, class_id)
        for student in update_list:
            if student['className'] == student:
                assert  student['description'] == "二年级数学学习班"
    ...
    def test_admin_school_positive_migrate_ok(self, school_fixture):
        ...
        for class_info in school_fixture:
            class_students_res = self.admin_school.class_students(self.authorization, class_info[0])
            res_ids = DataFrame(class_students_res["data"]["content"]).loc[:, "id"].tolist()
            assert res_ids == class_info[1]

        # 将classb与classc中的学生迁移到classa
        res = self.admin_school.migrate_student(self.authorization, [class_b_id, class_c_id], class_a_id)
        ...
        # 验证学生迁移成功
        students_a = self.admin_school.class_students(self.authorization, class_a_id)
        ...
        students_b = self.admin_school.class_students(self.authorization, class_b_id)
        assert not students_b['data']["content"]
        students_c = self.admin_school.class_students(self.authorization, class_c_id)
    ...
    def test_admin_school_positive_getLessons_all_ok(self, create_class):
        ...
        lesson_id = self.admin_school.create_lesson(self.authorization, classId=class_id)['data']['id']
        ...
        add_list = self.admin_school.class_lessons(self.authorization, class_id)
        ...
        update_res = self.admin_school.update_lesson(self.authorization, lesson_id, class_id, **update_pl)
        assert update_res['message'] == "success"
        # 更新班级信息后，获取班级列表，验证更新成功
        update_list = self.admin_school.class_lessons(self.authorization, class_id)
        ...
        self.admin_school.delete_lesson(self.authorization, lesson_id)
        # 删除班级后，获取班级列表，验证删除班级成功
        delete_list = self.admin_school.class_lessons(self.authorization, class_id)
        if delete_list:
            class_ids = DataFrame(delete_list).loc[:, "id"].tolist()
            assert class_id not in class_ids
    ...
    def test_admin_school_positive_admin_create_class1(self):
        ...
        studentIds = DataFrame(students_res).loc[:, "id"].tolist()
        # 更新班级学生默认分组
        res = self.admin_school.update_students_class_groups(self.authorization, classId=class_id, groupSeqNo=0, studentIds=studentIds)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"