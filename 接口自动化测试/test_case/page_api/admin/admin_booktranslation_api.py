...
class AdminBooktranslationApi(BaseAPI):
    ...
    def concurrent_threshold(self, authorization, DeviceType="web", code=200, **kwargs):
        """
        获取书籍翻译并发阈值设置
        ...
        error_msg = "获取书籍翻译并发阈值设置"
        assert response.status_code == code, f"{error_msg}失败，url->{url}，失败信息->{response.reason}{response.content}"
        response = response.json()
        return response

    def update_concurrent_threshold(self, authorization, threshold, DeviceType="web", code=200, **kwargs):
        """
        更新书籍翻译并发阈值设置
