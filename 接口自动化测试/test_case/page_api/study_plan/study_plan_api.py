...
class Study_planApi(BaseAPI):
    ...
    def claimReward(self, authorization, studyPlanId=0, kidId=0, DeviceType="web", code=200, **kwargs):
        """
        领取学习计划奖励
        :param studyPlanId: (integer, path, required) 学习计划ID
        :param request: (object, body, optional) 领取奖励请求
        :return: 接口原始返回（已 json 解析）
        """
        # Create Data:  V1.19.0  &  2025-10-24
        url = f"https://{base_url}/api/study-plan/{studyPlanId}/claim-reward"
        payload = {
          "kidId": kidId
        }
        payload = self.request_body(payload, **kwargs)
