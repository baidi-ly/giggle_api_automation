...
class TestBook:
    ...
    @pytest.mark.release
    def test_book_positive_uploadcover_ok(self):
        """修改封面-正向用例"""
        res = self.book.uploadcover(self.authorization)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    def test_book_permission_uploadcover(self, desc, value):
        """修改封面-权限测试"""
        # 鉴权作为位置参数直接传入（示例期望的极简风格）
        res = self.book.uploadcover(value, code=401)
        if res:
            assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
            assert res['code'] == 401, f"接口返回状态码异常: 预期【401】，实际【{res['code']}】"
            assert res['message'] == 'unauthorized', f"接口返回message信息异常: 预期【unauthorized】，实际【{res['message']}】"
    ...
    def test_book_required_uploadcover_bookId(self, desc, value, code):
        """修改封面-必填字段测试(bookId)"""
        if desc == 'missing':
            pl = {'pop_items': 'bookId'}
        else:
            pl = {'bookId': value}
        res = self.book.uploadcover(authorization=self.authorization, **pl, code=code)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        if code == 500:
            assert res['code'] == 500, f"接口返回状态码异常: 预期【500】，实际【{res['code']}】"
            assert res['message'] == 'internal server error', f"接口返回message信息异常: 预期【'internal server error'】，实际【{res['message']}】"
    ...
    def test_book_format_uploadcover_bookId(self, desc, value, code, code_res):
        """修改封面-数据格式测试(bookId)"""
        try:
            res = self.book.uploadcover(self.authorization, bookId=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def test_book_boundary_uploadcover_bookId(self, desc, value, code):
        """修改封面-边界值测试(bookId)"""
        res = self.book.uploadcover(self.authorization, bookId=value)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == code, f"接口返回状态码异常: 预期【{code}】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    @pytest.mark.release
    def test_book_scenario_uploadcover_invalid_bookId(self):
        """修改封面-场景异常-无效的bookId"""
        bookId = 999999999
        res = self.book.uploadcover(self.authorization, bookId=bookId)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    def test_book_required_uploadcover_file(self, desc, value, code):
        """修改封面-必填字段测试(file)"""
        if desc == 'missing':
            res = self.book.uploadcover(authorization=self.authorization, code=code)
        else:
            file = {
                'file': (value, open(os.getcwd() + f'/test_data/{value}', 'rb'))
            }
            res = self.book.uploadcover(authorization=self.authorization, file=file, code=code)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        if code == 500:
            assert res['code'] == 500, f"接口返回状态码异常: 预期【500】，实际【{res['code']}】"
            assert res['message'] == 'internal server error', f"接口返回message信息异常: 预期【'internal server error'】，实际【{res['message']}】"
    ...
    def test_book_format_uploadcover_file(self, desc, value, code, code_res):
        """修改封面-数据格式测试(file)"""
        try:
            file = {
                'file': (value, open(os.getcwd() + f'/test_data/{value}', 'rb'))
            }
            res = self.book.uploadcover(self.authorization, file=file, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
    ...
    def test_book_boundary_uploadcover_file(self, file):
        """修改封面-边界值测试(file)"""
        _audioFile = []
        for i in file:
            _audioFile.append((i[0], open(os.getcwd() + i[1], 'rb')))
        file = {"file": _audioFile}
        res = self.book.uploadcover(self.authorization, file=file)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
