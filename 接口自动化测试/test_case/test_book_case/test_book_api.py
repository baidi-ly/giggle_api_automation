...
class TestBook:
    ...
    @pytest.mark.release
    def test_book_positive_generate_ok(self, get_bookId):
        """生成故事书的quiz-正向用例"""
        bookId = get_bookId["data"]["content"][0]["id"]
        res = self.book.generate(self.authorization, bookId)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        assert res['code'] == 200, f"接口返回状态码异常: 预期【200】，实际【{res['code']}】"
        assert res['message'] == 'success', f"接口返回message信息异常: 预期【success】，实际【{res['message']}】"
        assert res['data'], f"接口返回data数据异常：{res['data']}"
    ...
    def test_book_permission_generate(self, desc, value):
        """生成故事书的quiz-权限测试"""
        # 鉴权作为位置参数直接传入（示例期望的极简风格）
        res = self.book.generate(value, code=401)
        if res:
            assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
            assert res['code'] == 401, f"接口返回状态码异常: 预期【401】，实际【{res['code']}】"
            assert res['message'] == 'unauthorized', f"接口返回message信息异常: 预期【unauthorized】，实际【{res['message']}】"
    ...
    def test_book_required_generate_content(self, desc, value, code, get_bookId):
        """生成故事书的quiz-必填字段测试(content)"""
        if desc == 'missing':
            pl = {'pop_items': 'content'}
        else:
            pl = {'content': value}
        bookId = get_bookId["data"]["content"][0]["id"]
        res = self.book.generate(self.authorization, bookId, code=code, **pl)
        assert isinstance(res, dict), f'接口返回类型异常: {type(res)}'
        if code == 500:
            assert res['code'] == 500, f"接口返回状态码异常: 预期【500】，实际【{res['code']}】"
            assert res['message'] == 'internal server error', f"接口返回message信息异常: 预期【'internal server error'】，实际【{res['message']}】"
    ...
    def test_book_format_generate_content(self, desc, value, code, code_res, get_bookId):
        """生成故事书的quiz-数据格式测试(content)"""
        try:
            bookId = get_bookId["data"]["content"][0]["id"]
            res = self.book.generate(self.authorization, bookId, content=value, code=code)
        except Exception as res:
            assert not code
        if code and not code_res:
            assert not res
